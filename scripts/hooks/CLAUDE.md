...
11 rules total (Next.js stack, post-migration — E282, +Rule 24 E345, +Rule 25 E351, +Rule 26 E368). The old
...
| 26 | Rows-Affected Before Audit (E368) — a function in `next-app/actions/*.ts` that runs an ownership-scoped `db.update(...)`/`db.delete(...)` AND writes an audit event (`logAudit(` or a `defineAction` `audit:` block) must inspect the rows-affected count. **Why:** a Server Action is a public POST endpoint; when the scoped WHERE matches zero rows (someone else's id, or a nonexistent one) the write is a no-op but the audit entry still lands — attributed to the caller, naming a resource they cannot touch. That is a forged write into the compliance surface E356/E359 hardened. `actions/items.ts` was already correct; EIGHT other functions were not, of which a code review had reported only four — the rule found the rest. Delegates the scan to `scripts/hooks/lib/rows-affected-scan.awk`. **Satisfied by** any of: capturing the result and bailing on `.count === 0` before the audit write; `.returning()` plus a null test; proving existence first with a SELECT (direct `db.select(` **or** a `await getSomething(...)` query helper) followed by an `if (!x) … return` guard, then writing by the proven id; or the explicit `// stop-verifier:rows-affected-ok` marker for a genuinely no-op-tolerant bulk write. **Exempt by design:** a write with no audit event at all (Rule 26 targets the forgery class only). **Scanner note:** drizzle chains across LINES (`await db` ⏎ `.update(...)`), so a single-line `db.update(` pattern matches nothing — the first draft did exactly that and reported a clean bill of health for all eight known-bad functions. The fixture in `test-rule-26-rows-affected.sh` line-breaks case 2 on purpose so that regression cannot return silently. Comment lines are skipped before matching, so neither a commented-out example counts as a write nor prose mentioning `.count` counts as a check. | Per file | exit 2 |
