...
1. 你新增的每一條路由（透過 `make new-domain` 或 `/athena:domain`）在你把它加進
   [`client/e2e/a11y.spec.ts`](../../../client/e2e/a11y.spec.ts) 的路由清單之前，**不會**進入 a11y 掃描。
2. 自訂主題 token 必須通過 **4.5:1**（一般文字）/ **3:1**（大型文字 + 非文字 UI）的對比度。
   見 [§3 的 token 對照表](#3-自訂-token-對比度目標)。
3. 純圖示按鈕需要 `aria-label`；對話框使用 `<Modal>` / `<Drawer>`；即時區域使用
   `role="alert"` / `aria-live`。見 [§4](#4-鍵盤--aria-撰寫規則)。
4. 本機執行閘門：`cd client && pnpm test:e2e --project=a11y`。只要有一個違規就會讓整次執行失敗。
...
| 保證 | 位置 |
|---|---|
| **WCAG 2.1 Level A + AA**，零違規 | `runAxe()` 以 `WCAG_TAGS = ["wcag2a", "wcag2aa", "wcag21a", "wcag21aa"]` 執行 axe，位於 [`client/e2e/helpers/a11y-runner.ts:22`](../../../client/e2e/helpers/a11y-runner.ts) |
| 掃描 **14 條路由**（public + auth + dashboard） | [`client/e2e/a11y.spec.ts:37-56`](../../../client/e2e/a11y.spec.ts) 的 `PUBLIC_PAGES` + `DASHBOARD_PAGES` |
| 掃描 **primitive 互動**（Modal + Toast） | [`client/e2e/a11y-primitives.spec.ts:50-104`](../../../client/e2e/a11y-primitives.spec.ts) |
| **靜態 landmark / ARIA** 檢查（22 個單元斷言） | [`client/src/tests/a11y/accessibility.test.tsx`](../../../client/src/tests/a11y/accessibility.test.tsx) |
| **閘門** — 只要一個違規就讓 PR 失敗 | `client/playwright.config.ts` 的 Playwright project `[a11y]`（`name: "a11y"`，`testMatch: /a11y(-[\w-]+)?\.spec\.ts/`） |
...
[`a11y.spec.ts:74`](../../../client/e2e/a11y.spec.ts) 與
...
```bash
# 本機執行 a11y 閘門（需要 server 在 :8080 + client dev 在 :3100）：
cd client
...
scaffold 一個 domain 時，產生器會在 `client/src/pages/{kebab-plural}/` 建立一個頁面，並提醒你把它的路由
接進 `App.tsx`。**在你把它加進掃描之前，那條路由對 a11y 掃描是隱形的。** 兩處編輯就能補上這個缺口。

### 2a. 把你的路由加進頁面掃描

dashboard 掃描會走訪 [`client/e2e/a11y.spec.ts:51-56`](../../../client/e2e/a11y.spec.ts) 的
...
Modal、Drawer、Toast 在被觸發前不會出現在 DOM 裡，所以頁面掃描搆不到它們 — 它們在
[`client/e2e/a11y-primitives.spec.ts`](../../../client/e2e/a11y-primitives.spec.ts) 裡有專屬的互動 spec。
**如果你打造了一個新的觸發式 primitive**（自訂 drawer、popover、command palette 等），請比照
[`a11y-primitives.spec.ts:50-71`](../../../client/e2e/a11y-primitives.spec.ts) 的 Modal 模式：

```ts
// client/e2e/a11y-primitives.spec.ts（放在現有的 describe 區塊內）
...
spec 會以 locator-not-found 錯誤大聲失敗，而不是默默通過。這正是現有 Modal spec 遵循的紀律（見其註解
[`a11y-primitives.spec.ts:44-49`](../../../client/e2e/a11y-primitives.spec.ts)）。

> **優先組合既有的 primitive。** 在動手寫新的觸發式 primitive 之前，先確認
> [`client/src/components/ui/`](../../../client/src/components/ui/) 裡的 `<Modal>` 或 `<Drawer>` 是否已
> 涵蓋你的需求。它們出廠就帶 focus-trap + ARIA 接線；自己手刻的 portal 沒有。這樣現有的 Modal/Toast
> 掃描就免費罩著你。
...
[`client/src/styles/themes.css`](../../../client/src/styles/themes.css) 裡的文字／表面 token 挑新的 hex 值。
這個檔案的標頭本身就在邀請你這麼做（「To create a custom theme: copy any block, change the selector and
colors」）。陷阱在於：因為跨主題掃描尚未實作（見 [§1](#1-模板已經保證了什麼) 的注意事項），
**沒有任何自動測試會檢查非預設主題的對比度。** 這些標準線由你負責。
...
（[`a11y-runner.ts:22`](../../../client/e2e/helpers/a11y-runner.ts）） — 你只是親手檢查矩陣搆不到的那些儲存格。

### 你必須檢查的 token 配對

`themes.css` 裡每個 `[data-theme="..."]` 區塊都定義了一組文字三元組疊在一對表面之上。這些才是實際會
...
> 對這題的解法是「bump the `--text-muted` token in `themes.css` for the offending theme」。請逐主題決定你的
> muted token 是承載文字還是只承載裝飾，並據此挑值。
...
「Adding a Theme」食譜，讓 47 個變數的契約保持完整 — 缺少的變數會 fallback 到 `:root`（dark），在淺色主題上
默默破壞對比度。
...
模板是 primitive-first 的：頁面組合 [`client/src/components/ui/`](../../../client/src/components/ui/) 並繼承這些
primitive 出廠的 ARIA 接線。為任何新的自訂元件遵循下列 DO/DON'T 規則，才不會重新引入 primitive 早已解決
的違規。

| 主題 | ✅ 該做 | ❌ 別做 |
|---|---|---|
| **純圖示按鈕** | 加上 `aria-label`。透過 `useTranslation('primitives')` 讓它可翻譯 — key 放在 [`client/src/locales/{lang}/primitives.json`](../../../client/src/locales/en/primitives.json)（例如 `modal.close`、`toast.dismiss`）。 | 出貨一個光禿禿的 `<button><Icon/></button>` — axe 會標記 `button-name`。 |
| **對話框／覆蓋層** | 使用 `<Modal>` 或 `<Drawer>`。它們出廠帶 `role="dialog"`、`aria-labelledby`（由 `title` prop 自動接線 — 見 [`Modal.tsx:77-79`](../../../client/src/components/ui/Modal.tsx)）與 focus trap。 | 手刻一個 portal `<div>`。沒有 focus trap = 鍵盤使用者會逃到覆蓋層後面的頁面。 |
| **即時區域**（toast、非同步 banner） | 使用 `<Toast>`（它設定 `aria-live` — 錯誤用 `assertive`，否則 `polite`，見 [`Toast.tsx:58`](../../../client/src/components/ui/Toast.tsx)），或 render 一個 `[role="alert"]` banner。 | 更新畫面上的狀態文字卻沒有 live-region role — 螢幕報讀器永遠不會播報它。 |
| **表單輸入** | 把每個 input 包進 `<FormField>`（它會關聯 `<label for>` + 為錯誤關聯 `aria-describedby`）。 | 用一個裸 `<input>` 配一個浮動的 `<span>` 標籤 — axe 會標記 `label`。 |
| **Tab 順序** | 維持邏輯 DOM 順序：**skip-nav → topbar → nav → main**。app 出廠帶 `<SkipNav>`（[`client/src/components/SkipNav.tsx`](../../../client/src/components/SkipNav.tsx)）指向 `#main-content`；把那個 id 給你頁面的 main 區域。 | 用 `tabindex` > 0 來「修正」順序 — 它會讓視覺與鍵盤順序脫鉤。只有 `0` / `-1` 是可接受的。 |
...
無障礙名稱（accessible name）是面向使用者的字串，所以它遵循專案的雙語規則：每個 `aria-label` 預設值都放在
`en/` 與 `zh-TW/` 兩邊的 `primitives.json`，透過 `useTranslation('primitives')` 存取。**呼叫端傳入的 prop 永遠
勝過** i18n 預設 — 例如 primitive 會寫 `aria-label={props.label ?? t('search.placeholder')}`。所以當你撰寫自訂
元件時：

- 把預設的無障礙名稱**同時**放進 `locales/en/primitives.json` **與** `locales/zh-TW/primitives.json`
  （key 相同，值翻譯）。
