...
| Phase 89 | E367, E368, E369, E370, E371 | 🟢 APPROVED (2026-09-09) — **產品程式碼稽核修補**（來源：2026-09-09 `next-app/` code-review，10 個 finding 全部經逐一查證，無虛報；其中 2 個 reviewer 描述有誤已更正 —— F2 非永久鎖死而是 15 分鐘窗口、F7 該動作並非 public）。**貫穿主軸：個案修補未推及同類。** E368(F3) 與 E369(F9) 是同一句話的兩個面 —— `items.ts` 的正確寫法連註解都寫好了，隔壁五個檔案沒跟上；E348/E352 刪掉 items 的影子驗證器，其他三個入口原封不動。E367(F1) 則是 E350 那類草稿外洩在 E333 custom 路徑上的殘留。**唯一 HIGH：E367** —— `/p/[slug]` 對 custom registry 命中就無條件渲染，不讀 `status`；而 `canServeSalesPageRow` 對 custom 直接回 false（讓位給 registry）—— 兩側都以為對方在管。admin UI 完整支援對 custom 列按取消發佈，動作回成功、寫稽核、revalidate，頁面照樣公開。**Wave 1 = E367 + E368 + E370 + E371（檔案互斥）→ Wave 2 = E369**（依賴 E368，共用 api-keys/webhooks/team 三檔）。E371 觸及 auth+DB，publish 須帶 `--e2e`。 |
