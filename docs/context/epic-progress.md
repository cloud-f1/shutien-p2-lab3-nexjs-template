...
| Phase 90 | E372, E373, E374, E375 | 🟢 APPROVED (2026-09-09) — **Phase 89 稽核的直接產物**。E372 是 fast-follow：E370 在 `defineAction` public 路徑上無條件呼叫 `headers()`，無 request scope 時拋錯，打掛 `checkout.int.test.ts` 全部訪客路徑案例 —— **通過了五道全綠閘門才被稽核抓到**，因為 `pre-merge-check.sh` 只跑 `pnpm test`（unit），`test:int` 只存在於 `make verify` 而發布路徑從不呼叫它；且 E370 自己新寫的測試 mock 了 `next/headers`，把它要涵蓋的脆弱性遮住。E373 = e2e 種子狀態隔離（本 phase 被 2FA 殘留擊倒三次，11→16 個失敗的自我放大）。E374 = VRT 基準線決策（4 個快照自 2026-06-15 起即紅，且不在閘門內 —— 需人判斷是重產快照還是真實視覺回歸）。E375 = 限流可觀測性：`rateLimitGuard` 只回錯誤字串不記錄事件，公開結帳被擋時 fork 營運者看不到任何訊號，且 10 次/分鐘/IP 對 CGNAT 後方的真實買家可能過嚴（失敗模式是靜默的營收損失）。 |
