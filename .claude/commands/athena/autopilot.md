...
3. **Execute the step** (only when advance was granted):
   ...
   - `merge` → inline (Bash) — the **Publish step (auto-merge by default)** from
     `loop.md`: `git push` + `gh pr create` + `gh pr merge` after the mandatory gates;
     on merge failure degrade to `⏸ awaiting human merge (PR #N)`, never force.
     With `ATHENA_AUTO_MERGE=0` stop at `⏸ awaiting human merge (PR #N)` + EXIT and
     the USER merges.
